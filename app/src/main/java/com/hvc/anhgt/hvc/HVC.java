...
public abstract class HVC {
    ...
    protected int Execute(int inTimeOutTime, int inExec, byte[] outStatus, HVC_RES result) {
        ...
        for (i = 0; i < numBody; i++) {
            HVC_RES.DetectionResult body = new HVC_RES().new DetectionResult();
        ...
        for (i = 0; i < numHand; i++) {
            HVC_RES.DetectionResult hand = new HVC_RES().new DetectionResult();
        ...
        for (i = 0; i < numFace; i++) {
            HVC_RES.FaceResult face = new HVC_RES().new FaceResult();
